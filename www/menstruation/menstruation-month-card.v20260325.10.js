...
class MenstruationMonthPanel extends HTMLElement {
  ...
  _render() {
    ...
    const style = `
      :host { display:block; }
      .panel { border-radius: 16px; overflow:hidden; background: var(--ha-card-background, var(--card-background-color, #111)); }
      ...
      .grid { padding: 0 10px 12px; }
      .weekdays { display:grid; grid-template-columns:repeat(7,1fr); gap: 6px; padding: 6px 0 10px; opacity:.9; font-size: 15px; font-weight: 800; text-align:center; }
      .weeks { display:grid; grid-template-rows:repeat(6,1fr); gap: 6px; }
      .week { display:grid; grid-template-columns:repeat(7,1fr); gap: 6px; }
      .cell { position:relative; height: 50px; border-radius: 14px; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size: 20px; font-weight: 900; }
